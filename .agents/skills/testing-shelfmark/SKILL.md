...
- Canonical "first book" of a series = `ORDER BY position, year, id` with the `wikidata_id NOT IN (SELECT wikidata_id FROM series…)` sub-entry exclusion plus a dup-position year re-sort. Any feature that independently derives a series' first book (compare, studies, FAQ) must be cross-checked against the series page's Start-here row; Discworld is a good tripwire fixture. /compare pairs are alphabetical by localeCompare; reversed slugs 301 only when both series exist.
- Series pages of genre top-8 series carry a "Can't decide?" compare-pill row inside the print-hidden "If you like…" section (locate via the `Can't decide?` span; siblings are the pills). Compare pair URLs are canonical only in alphabetical order — probe pill hrefs with a no-redirect opener to prove they don't bounce through a 301.
- Backup nudge (`maybeBackupHint`, public/app.js): fires on a tick only when the shelf was NOT previously empty and global tracked count ≥5; inserts a `role="note"` div directly after `ol[data-series]`; sets `shelfmark_hint_backup_v1` on SHOW (not dismiss) — each fresh profile gets exactly one shot, so use a fresh Playwright context per scenario. `/tmp/axe.min.js` can vanish between sessions (tmp cleanup) — re-fetch from cdnjs axe-core 4.10.2 if missing.
- Backup nudge final structure: `ol[data-series] > li.list-none.print:hidden > div[role=note].coach-tip` — locate via `[role=note]`, but assert print-hiding and removal on the OUTER li (parentElement). When auditing anything injected into the series `<ol>`, run axe with it visible AND after dismissal (dismissal = clean control; the `list` rule flags any non-listitem child of ol).
- Lighthouse scores are only trustworthy when run SEQUENTIALLY on an idle box — `pkill -f chrome` first, never alongside other browser lanes (parallel contention has faked ~10 Perf points / +1s LCP regressions). Sequential mobile baseline (worker 2ea22620): Perf 97 / LCP ~2.4s / CLS 0.001 / TBT 0. `npm install -g lighthouse`, then `lighthouse <url> --only-categories=performance --chrome-flags="--headless=new" --output=json`.
- Static assets are versioned: HTML references `/app.js?v=<8-hex>` and `/styles.css?v=<8-hex>` (hashes generated into `src/asset-versions.ts` by `scripts/asset-versions.mjs` during `npm run css`). When verifying a deploy, extract the current hashes from any page's HTML rather than hardcoding, and check the bare `/app.js` / `/styles.css` URLs still return 200 with byte-identical bodies (old cached HTML must not break).
- Coach-tips (`.coach-tip`) animate `coach-tip-in` 0.9s (fade-in + amber border pulse) only under `prefers-reduced-motion: no-preference` — use Playwright's `reduced_motion` context option to test both paths, and run axe immediately after insertion to catch mid-animation contrast issues (currently clean).
- Letter hub pages (`/series?letter=X`, `/authors?letter=X`) carry a full link index (`letterIndexSection`) on page 1 only — assert heading count == li count == meta-description count, and absence on page 2 / unlettered hubs. The sitemap's regression fingerprint: 10 shards summing to ~45,233 URLs, shard 1 holding the core block (letter hubs, /compare, /studies, genres, similar, compare pairs). IndexNow full push runs in the worker's weekly cron (runIndexNow), no manual script.
- Analytics beacons POST to `/api/hit` with newline-delimited payload `path\nrefHost(\nvisit)?` — test client behavior by intercepting `page.on("request")` post_data (server drops HeadlessChrome/DevinQA UAs, so D1 won't show your hits). QA sessions can suppress the beacon entirely with `localStorage.shelfmark_qa="1"` — set it in long GUI walkthroughs to avoid polluting production analytics. Visit-state keys: `shelfmark_first_v1` (first-seen day) and `shelfmark_evday_v1` (per-day guard).

## R8 notes (edge cache + toast)
- Edge cache emits `X-Edge-Cache: MISS/HIT` on public GET pages; cache key includes the full query string, so a unique `?cb=` param gives a guaranteed-fresh MISS→HIT pair without polluting real keys. Excluded paths must never carry the header: `/api/*`, `/random`, `/confirm`, `/unsubscribe`, `/search`, `/shelf`.
- The first-visit tip is a fixed bottom toast (`.coach-tip[role=note]`, key `shelfmark_tip_track_v1`) appended inside `<main>` (fallback body) so axe's `region` rule stays clean — when auditing any dynamically injected floating element, assert its landmark parent, not just its role. It swaps to role=status after the first tick and auto-removes in 12s. Use post-dismissal axe as the control; toast screenshots must wait ~1.5s for the fade-in or the background reads as transparent.

## R9 notes (numbered order vs extras)
- Series pages may render TWO `ol[data-series]` with the same slug: the numbered main list plus an extras list inside `<details>` ("Companions, novellas & shorts (N)"). The progress label/bar counts only the FIRST list (`updateSeriesUI` uses `querySelector`), and `updateUpNext` skips lists inside `<details>` — target lists explicitly with `details ol[data-series]` vs the first match. Extras rows have no ordinal span and sort by year; ticks in extras still persist and count on /shelf.
- Prequels carry ordinal <1, render a `year-chip` "Prequel" badge, and never get "Start here" — `startBook` picks the first position≥1 book. Good fixtures: `discworld` (41 numbered + 16 extras, starts Colour of Magic 1983), `the-wheel-of-time` (New Spring at 0), `rivers-of-london` (1–10 incl. 5.5 novella).
- Data integrity is gated by `scripts/audit_series_integrity.py` against `docs/data-integrity-baseline.json` (ratchet: counts may only fall); it runs on a local sqlite mirror built from `wrangler d1 export` dumps.
