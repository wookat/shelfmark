      ...
      Object.keys(bySeries).forEach(function (k) {
        ...
        html += '<section class="mb-6"><h2 class="font-display font-semibold text-xl text-ink-900">' +
          ...
          g.items.map(function (e) {
            return '<li class="flex items-center justify-between rounded-xl bg-white border border-ink-200 px-4 py-2.5 text-sm"><span class="font-medium text-ink-900">' + escapeHtml(e.title || e.id) + '</span><span class="text-ink-700/60">' + (e.t > 1e12 ? new Date(e.t).toLocaleDateString() : "") + "</span></li>";
          }).join("") + "</ul></section>";
      });
      root.innerHTML = html;
    }
